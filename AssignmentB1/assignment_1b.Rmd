...
For this assignment, I'd like to use the function I will create to answer questions pertaining to the zooplankton_biomass dataset from DFO. I will use a cleaned up version of that I created for the Mini Data Analysis Milestone 3 assignment from the STAT545A class (below code from STAT545A MDA Milestones 2-3).
```{r}
...
I want to create a function that will allow me to compare the proportion of total biomass made up by one of the zooplankton taxa pre and post heat wave. I will make this function general so that it can be used to compare any numeric variable across each category of any categorical variable. 
...
Finally, let's use the boxplot_numeric_category function to create a boxplot that compares Copepod biomass pre and post heat wave, as this was something I was interested in from the STAT545A MDA Milestones.
```{r}
...
First, let's test to see if Examples 1-3 above, which have the right variable types as inputs, run without an error message. Let's also test to see if Example 4, which did not have all correct variable class types, gave the error message specified in the boxplot_numeric_category function code: 
```{r}
...
Now let's test now to see if each of Examples 1-3 and the Zooplankton Biomass data example, which all use the correct variable class types in the boxplot function, output a ggplot:
```{r}
...
Joels. 2018. Converting dates (Year - Month - Day) to 3 separate columns (Year , Month , Day). Studio Community; [modified 2018 May 18; accessed 2021 Oct 19]. https://community.rstudio.com/t/converting-dates-year-month-day-to-3-separate-columns-year-month-day/8585/4