...
For this assignment, I'd like to use the function I will create to answer questions pertaining to the zooplankton_biomass dataset from DFO. I will use a cleaned up version of that I created for the Mini Data Analysis Milestone 3 assignment from the STAT545A class.
```{r}
...
```
I want to further modify this cleaned dataset to work best with the function I plan to create:
```{r}
...
calculations_y <- summarise(dataframe, is.numeric = is.numeric({{y}}) | is.double({{y}}) | is.integer({{y}}), class = class({{y}}))
  ...
  if(!calculations_x$is.character) {
    stop('You have entered a non-character or non-factor input. Please enter a character variable or factor variable for the x input', class(calculations_x$class))
  }
  
  if(!calculations_y$is.numeric) {
    stop('You have entered a non-numeric or non-factor input. Please enter a character variable or factor variable for the x input', class(calculations_y$class))
  }
...
Now let's run this function see if it will create a boxplot that compares Copepod biomass pre and post heat wave.
```{r}
...
First, let's test to see if the three examples that have the right variable types run without an error message and if the one example that did not have all correct variable types gave an error message: 
```{r}
...
Let's test now to see if each of the examples that use the correct variable types in the boxplot function output a ggplot (this is what we would expect):
```{r}
...
```{r}
  ...
  #calculations_x <- summarise(dataframe, is.character = is.character({{y}}) | is.factor({{x}}), class = class({{x}}))
  
  #calculations_y <- summarise(dataframe, is.numeric = is.numeric({{y}}), is.double = is.double({{y}}), is.integer = #is.integer({{y}}), class = class({{y}}))
  ...
  #if(!calculations_x$is.character || !calculations_x$is.factor) {
    #stop('You have entered a non-character or non-factor input. Please enter a character variable or factor variable for #the x input', class(calculations_x$class))
  #}
  
  #if(!calculations_y$is.numeric || !calculations_y$is.integer || !calculations_y$is.double) {
    #stop('You have entered a non-character or non-factor input. Please enter a character variable or factor variable for #the x input', class(calculations_y$class))
 # }
