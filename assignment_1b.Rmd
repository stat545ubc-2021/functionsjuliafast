...
**Install** `tidyverse`, `readr`, and `roxygen2` packages if you have not already done so:
```{r, message = FALSE}
...
**Load** the `tidyverse` and `roxygen2` packages, as well as the `zooplankton dataset` being used for this project:
```{r, message = FALSE}
...
I want to create a function that compares the proportion of total biomass made up by one of the zooplankton taxa pre and post heat wave.
...
#' @params data The dataframe that contains the variables that you would like to use to create the bozplot
